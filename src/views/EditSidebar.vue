	...
	>
		<template v-if="isLoading">
			<div class="app-sidebar__loading-indicator">
				<div
					class="icon icon-loading app-sidebar-tab-loading-indicator__icon"
				/>
			</div>
		</template>
		...
		<template v-if="!isLoading && !isError && !isNew" #secondary-actions>
			<NcActionLink
		...
		<template v-if="!isLoading && !isError" #description>
					...
					<div class="modal-content">
						...
						<div class="modal-h">
							{{
								n(
									"calendar",
									"User requires access to your file",
									"Users require access to your file",
									showModalUsers.length
								)
							}}
						</div>
						<div class="users">
						...
						<div class="modal-subtitle">
							{{
								n(
									"calendar",
									"Attachment requires shared access",
									"Attachments requiring shared access",
									showModalNewAttachments.length
								)
							}}
						</div>
		...
		<NcAppSidebarTab
			v-if="!isLoading && !isError && showResources"
...
import PropertyTitleTimePicker2 from "../components/Editor/Properties/PropertyTitleTimePicker2.vue";
...
export default {
	...
	computed: {
		...
		subTitle() {
			...
			const userTimezone = getTimezoneManager().getTimezoneForId(
			...
			const startDateInUserTz =
				this.calendarObjectInstance.eventComponent.startDate.getInTimezone(
					userTimezone
				).jsDate;
	...
	methods: {
		...
		isPrivate() {
			return (
				this.calendarObjectInstance.attendees.filter((attendee) => {
					if (
		...
		getPreview(attachment) {
			if (attachment.xNcHasPreview) {
				return generateUrl(
					`/core/preview?fileId=${attachment.xNcFileId}&x=100&y=100&a=0`
				);
			}
					...
					this.showModalNewAttachments.map(async (attachment, i) => {
						...
						try {
							const data = await shareFile(
								`${this.attachmentsFolder}${attachment.fileName}`
							);
							attachment.shareTypes = data?.share_type?.toString();
							if (
								typeof attachment.attachmentProperty.getParameter(
									"X-NC-SHARED-TYPES"
								) === "undefined"
							) {
		...
		prepareAccessForAttachments(thisAndAllFuture = false) {
			...
			const newAttachments = this.calendarObjectInstance.attachments.filter(
				(attachment) => {
					...
					return !this.isPrivate()
						? attachment.isNew && attachment.shareTypes === null
						: attachment.isNew && attachment.shareTypes !== null;
				}
			);
			...
			if (newAttachments.length > 0 && !this.isPrivate()) {
				...
				this.showModalUsers = this.calendarObjectInstance.attendees.filter(
					(attendee) => {
						if (
							this.currentUser.emailAddress.toLowerCase() !==
							this.emailWithoutMailto(attendee.uri)
						) {
							return attendee;
						}
						return false;
		...
		saveEvent(thisAndAllFuture = false) {
			...
			this.calendarObjectInstance.attachments =
				this.calendarObjectInstance.attachments.map((attachment) => {
					if (attachment.isNew) {
