...
const mutations = {
	...
	setCalendarObjectInstanceForExistingEvent(
		state,
		{ calendarObject, calendarObjectInstance, objectId, recurrenceId }
	) {
		state.isNew = false;
	...
	changeStartDate(state, { calendarObjectInstance, startDate }) {
		calendarObjectInstance.eventComponent.startDate.year =
			startDate.getFullYear();
		calendarObjectInstance.eventComponent.startDate.month =
		...
		calendarObjectInstance.eventComponent.startDate.minute =
		...
		const startDateObj =
			calendarObjectInstance.eventComponent.startDate.clone();
		...
		if (isAllDay) {
			...
			if (endDateObj.compare(startDateObj) === -1) {
				const timezone = getTimezoneManager().getTimezoneForId(
					endDateObj.timezoneId
				);
				calendarObjectInstance.eventComponent.endDate =
					calendarObjectInstance.eventComponent.startDate.getInTimezone(
						timezone
					);
				calendarObjectInstance.endDate = getDateFromDateTimeValue(
					calendarObjectInstance.eventComponent.endDate
				);
				calendarObjectInstance.eventComponent.endDate.addDuration(
					DurationValue.fromSeconds(60 * 60 * 24)
				);
			}
		} else {
			if (endDateObj.compare(startDateObj) === -1) {
	...
	changeStartTimezone(state, { calendarObjectInstance, startTimezone }) {
		...
		if (
			startTimezone === "floating" ||
			calendarObjectInstance.endTimezoneId === "floating"
		) {
	...
	changeEndDate(state, { calendarObjectInstance, endDate }) {
		...
		calendarObjectInstance.eventComponent.endDate.month =
			endDate.getMonth() + 1;
		...
		if (isAllDay) {
			...
			if (endDateObj.compare(startDateObj) === -1) {
				...
				if (endDateObj.compare(startDateObj) === -1) {
					...
					calendarObjectInstance.eventComponent.startDate =
						calendarObjectInstance.eventComponent.endDate.getInTimezone(
							timezone
						);
					calendarObjectInstance.startDate = getDateFromDateTimeValue(
						calendarObjectInstance.eventComponent.startDate
	...
	changeEndTimezone(state, { calendarObjectInstance, endTimezone }) {
		...
		// Either both are floating or both have a timezone, but it can't be mixed
		if (
			endTimezone === "floating" ||
			calendarObjectInstance.startTimezoneId === "floating"
		) {
			calendarObjectInstance.eventComponent.startDate.replaceTimezone(timezone);
	...
	toggleAllDay(state, { calendarObjectInstance }) {
		...
		calendarObjectInstance.isAllDay =
			calendarObjectInstance.eventComponent.isAllDay();

		// isAllDay = old value
		if (isAllDay) {
			calendarObjectInstance.eventComponent.endDate.addDuration(
				DurationValue.fromSeconds(-1 * 60 * 60 * 24)
			);
		} else {
			calendarObjectInstance.eventComponent.endDate.addDuration(
				DurationValue.fromSeconds(60 * 60 * 24)
	...
	changeLocation(state, { calendarObjectInstance, location }) {
		...
		calendarObjectInstance.eventComponent.deleteAllProperties(
	...
	changeDescription(state, { calendarObjectInstance, description }) {
		...
		const descriptionProperty =
			calendarObjectInstance.eventComponent.getFirstProperty("Description");
	...
	removeAttendee(state, { calendarObjectInstance, attendee }) {
		calendarObjectInstance.eventComponent.removeAttendee(
	...
	setOrganizer(state, { calendarObjectInstance, commonName = null, email }) {
		calendarObjectInstance.eventComponent.setOrganizerFromNameAndEMail(
			commonName,
			email
		);
		...
		Vue.set(
			...
			"attendeeProperty",
			calendarObjectInstance.eventComponent.getFirstProperty("ORGANIZER")
		);
	},
	...
	changeRecurrenceInterval(
		state,
		{ calendarObjectInstance, recurrenceRule, interval }
	) {
	...
	changeRecurrenceFrequency(
		state,
		{ calendarObjectInstance, recurrenceRule, frequency }
	) {
		if (recurrenceRule.recurrenceRuleValue) {
	...
	changeRecurrenceCount(
		state,
		{ calendarObjectInstance, recurrenceRule, count }
	) {
		if (recurrenceRule.recurrenceRuleValue) {
	...
	changeRecurrenceUntil(
		state,
		{ calendarObjectInstance, recurrenceRule, until }
	) {
		if (recurrenceRule.recurrenceRuleValue) {
			...
			if (calendarObjectInstance.startTimezoneId !== "floating") {
				recurrenceRule.recurrenceRuleValue.until = DateTimeValue.fromJSDate(
					until,
					{ zone: "utc" }
				);
			} else {
				recurrenceRule.recurrenceRuleValue.until =
					DateTimeValue.fromJSDate(until);
			}
	 ...
	 * @param {object} data.recurrenceRule The recurrenceRule object to modify
	 */
	setDefaultRecurrenceByPartsForWeekly(
		state,
		{ calendarObjectInstance, recurrenceRule }
	) {
		if (recurrenceRule.recurrenceRuleValue) {
	 ...
	 * @param {object} data.recurrenceRule The recurrenceRule object to modify
	 */
	setDefaultRecurrenceByPartsForMonthly(
		state,
		{ calendarObjectInstance, recurrenceRule }
	) {
		if (recurrenceRule.recurrenceRuleValue) {
			const byMonthDay = calendarObjectInstance.startDate.getDate().toString();
			recurrenceRule.recurrenceRuleValue.setComponent("BYMONTHDAY", [
				byMonthDay,
			]);
	...
	setDefaultRecurrenceByPartsForMonthlyBySetPosition(
		...
		if (recurrenceRule.recurrenceRuleValue) {
			const { byDay, bySetPosition } = getBySetPositionAndBySetFromDate(
				calendarObjectInstance.startDate
			);
			recurrenceRule.recurrenceRuleValue.setComponent("BYDAY", [byDay]);
	 ...
	 * @param {object} data.recurrenceRule The recurrenceRule object to modify
	 */
	setDefaultRecurrenceByPartsForYearly(
		state,
		{ calendarObjectInstance, recurrenceRule }
	) {
		if (recurrenceRule.recurrenceRuleValue) {
	...
	addByDayToRecurrenceRule(
		state,
		{ calendarObjectInstance, recurrenceRule, byDay }
	) {
		if (recurrenceRule.recurrenceRuleValue) {
			const byDayList =
				recurrenceRule.recurrenceRuleValue.getComponent("BYDAY");
	...
	addByMonthDayToRecurrenceRule(
		state,
		{ calendarObjectInstance, recurrenceRule, byMonthDay }
	) {
		if (recurrenceRule.recurrenceRuleValue) {
			const byMonthDayList =
				recurrenceRule.recurrenceRuleValue.getComponent("BYMONTHDAY");
			const index = byMonthDayList.indexOf(byMonthDay);
			if (index === -1) {
				byMonthDayList.push(byMonthDay);
				recurrenceRule.recurrenceRuleValue.setComponent(
					"BYMONTHDAY",
					byMonthDayList
				);
			}
	...
	addByMonthToRecurrenceRule(
		state,
		{ calendarObjectInstance, recurrenceRule, byMonth }
	) {
		if (recurrenceRule.recurrenceRuleValue) {
			console.debug("addByMonthToRecurrenceRule", byMonth);

			const byMonthList =
				recurrenceRule.recurrenceRuleValue.getComponent("BYMONTH");
	...
	changeRecurrenceBySetPosition(
		state,
		{ calendarObjectInstance, recurrenceRule, bySetPosition }
	) {
		if (recurrenceRule.recurrenceRuleValue) {
			recurrenceRule.recurrenceRuleValue.setComponent("BYSETPOS", [
				bySetPosition,
	...
	removeRecurrenceRuleFromCalendarObjectInstance(
		...
		if (recurrenceRule.recurrenceRuleValue) {
			...
			Vue.set(
				calendarObjectInstance,
				"recurrenceRule",
				getDefaultEventObject().recurrenceRule
			);
	 ...
	 * @param {object} data.calendarObjectInstance The calendarObjectInstance object
	 */
	addRecurrenceRuleFromCalendarObjectInstance(
		state,
		{ calendarObjectInstance }
	) {
	...
	changeAlarmHoursMinutesAllDay(
		state,
		{ calendarObjectInstance, alarm, hours, minutes }
	) {
		if (alarm.alarmComponent) {
			alarm.alarmComponent.trigger.value.totalSeconds =
				getTotalSecondsFromAmountHourMinutesAndUnitForAllDayEvents(
	...
	changeAlarmAbsoluteTimezoneId(
		state,
		{ calendarObjectInstance, alarm, timezoneId }
	) {
		if (alarm.alarmComponent) {
	...
	changeAlarmFromRelativeToAbsolute(state, { calendarObjectInstance, alarm }) {
		if (alarm.alarmComponent) {
			...
			alarm.absoluteDate = getDateFromDateTimeValue(
				alarm.alarmComponent.trigger.value
			);
	...
	changeAlarmFromAbsoluteToRelative(state, { calendarObjectInstance, alarm }) {
		if (alarm.alarmComponent) {
			const duration =
				alarm.alarmComponent.trigger.value.subtractDateWithTimezone(
					calendarObjectInstance.eventComponent.startDate
				);
	...
	addAlarmToCalendarObjectInstance(
		state,
		{ calendarObjectInstance, type, totalSeconds }
	) {
	...
	addAttachmentBySharedData(state, { calendarObjectInstance, sharedData }) {
		...
		const xNcHasPreview = new Parameter(
			"X-NC-HAS-PREVIEW",
			sharedData["has-preview"].toString()
		);
		// custom file id parameter from dav file
		const xNcFileId = new Parameter(
			"X-NC-FILE-ID",
			sharedData.fileid.toString()
		);
		// custom share-types parameter from dav file
		const xNcSharedTypes = new Parameter(
			"X-NC-SHARED-TYPES",
			sharedData["share-types"]["share-type"]
		...
		attachment.uri = sharedData.url
			? sharedData.url
			: generateUrl(`/f/${sharedData.fileid}`);
	...
	addAttachmentWithProperty(state, { calendarObjectInstance, sharedData }) {
		...
		const fileName = sharedData.fileName;
		attachment.isNew = true;
		attachment.shareTypes =
			(typeof sharedData?.["share-types"]?.["share-type"] === "number"
		...
		const attachmentProperty = AttachmentProperty.fromLink(
			attachment.uri,
		...
		const xNcHasPreview = new Parameter(
			"X-NC-HAS-PREVIEW",
			attachment.xNcHasPreview.toString()
		);
		const xNcFileId = new Parameter(
			"X-NC-FILE-ID",
			attachment.xNcFileId.toString()
		);
	...
	deleteAttachment(state, { calendarObjectInstance, attachment }) {
		try {
			...
			calendarObjectInstance.eventComponent.removeAttachment(
...
const actions = {
	...
	async resolveClosestRecurrenceIdForCalendarObject(
		...
		return firstVObject.recurrenceManager
			.getClosestOccurrence(d)
			.getReferenceRecurrenceId().unixTime;
	},
	...
	async getCalendarObjectInstanceByObjectIdAndRecurrenceId(
		...
		const calendarObject = await dispatch("getEventByObjectId", { objectId });
		const eventComponent = getObjectAtRecurrenceId(
			calendarObject,
			recurrenceIdDate
		);
	...
	async getCalendarObjectInstanceForNewEvent(
		...
		if (!isNaN(defaultReminder)) {
			...
			logger.debug(
				`Added defaultReminder (${defaultReminder}s) to newly created event`
			);
		}
	...
	async saveCalendarObjectInstance(
		...
		if (eventComponent.isDirty()) {
			...
			if (isForkedItem && eventComponent.canCreateRecurrenceExceptions()) {
				[original, fork] =
					eventComponent.createRecurrenceException(thisAndAllFuture);
			}
	 ...
	 * @return {Promise<void>}
	 */
	async deleteCalendarObjectInstance(
		{ state, dispatch, commit },
		{ thisAndAllFuture }
	) {
		const eventComponent = state.calendarObjectInstance.eventComponent;
		const isRecurrenceSetEmpty =
			eventComponent.removeThisOccurrence(thisAndAllFuture);
		const calendarObject = state.calendarObject;
	...
	changeStartDate({ commit }, { calendarObjectInstance, startDate }) {
		...
		const difference =
			startDate.getTime() - calendarObjectInstance.startDate.getTime();
		const endDate = new Date(
			calendarObjectInstance.endDate.getTime() + difference
		);
	 ...
	 * @param {object} data2.recurrenceRule The recurrenceRule object to modify
	 */
	disableYearlyRecurrenceBySetPosition(
		{ commit },
		{ calendarObjectInstance, recurrenceRule }
	) {
	...
	enableRecurrenceLimitByUntil(
		...
		switch (recurrenceRule.frequency) {
			...
			case "DAILY":
				until = new Date(
					calendarObjectInstance.startDate.getTime() + 7 * 24 * 60 * 60 * 1000
				);
				break;
			...
			case "WEEKLY":
				until = new Date(
					calendarObjectInstance.startDate.getTime() +
						4 * 7 * 24 * 60 * 60 * 1000
				);
				break;
	...
	changeAlarmAmountTimed(
		{ commit },
		{ calendarObjectInstance, alarm, amount }
	) {
