...
const mutations = {
	...
	removeDeletedCalendar(state, { calendar }) {
		state.deletedCalendars = state.deletedCalendars.filter(
			(c) => c !== calendar
		);
	},
	...
	removeDeletedCalendarObject(state, { vobject }) {
		state.deletedCalendarObjects = state.deletedCalendarObjects.filter(
			(vo) => vo.id !== vobject.id
		);
	},
	...
	appendCalendarObjectsToCalendar(state, { calendar, calendarObjectIds }) {
		for (const calendarObjectId of calendarObjectIds) {
			if (
				state.calendarsById[calendar.id].calendarObjects.indexOf(
					calendarObjectId
				) === -1
			) {
	...
	deleteFetchedTimeRangeFromCalendar(state, { calendar, fetchedTimeRangeId }) {
		const index =
			state.calendarsById[calendar.id].fetchedTimeRanges.indexOf(
				fetchedTimeRangeId
			);
	...
	shareCalendar(
		state,
		{ calendar, user, displayName, uri, isGroup, isCircle }
	) {
		const newSharee = {
			displayName,
			id: user,
	...
	unshareCalendar(state, { calendar, uri }) {
		...
		const shareIndex = calendar.shares.findIndex(
			(sharee) => sharee.uri === uri
		);
...
const getters = {
		...
		return state.calendars
			.filter(
				(calendar) =>
					...
					(rootState.settings.showTasks && calendar.supportsTasks)
			)
	...
	hasTrashBin(state) {
		return (
			state.trashBin !== undefined && state.trashBin.retentionDuration !== 0
		);
	},
	...
	deletedCalendarObjects(state) {
		...
		return state.deletedCalendarObjects.map((obj) => ({
			calendar:
				calendarUriMap[obj.dav._props["{http://nextcloud.com/ns}calendar-uri"]],
			...obj,
		}));
	},
	 ...
	 * @return {function({String}): {Object}}
	 */
	getCalendarByUrl: (state) => (url) =>
		state.calendars.find((calendar) => calendar.url === url),
	...
	sortedCalendarFilteredByComponents:
		(state, getters) => (vevent, vjournal, vtodo) => {
...
const actions = {
	...
	async loadCollections({ commit, state, getters }) {
		const { calendars, trashBins, scheduleInboxes, subscriptions } =
			await findAll();
		console.info("calendar home scanned", calendars, trashBins, subscriptions);
		calendars
			.map((calendar) =>
				mapDavCollectionToCalendar(calendar, getters.getCurrentUserPrincipal)
			)
			.forEach((calendar) => {
	...
	async loadDeletedCalendarObjects({ commit, state }) {
		...
		vobjects.forEach((vobject) => {
			try {
				const calendarObject = mapCDavObjectToCalendarObject(
					vobject,
					undefined
				);
	 ...
	 * @return {Promise}
	 */
	async appendCalendar(
		context,
		{ displayName, color, order, components = ["VEVENT"], timezone = null }
	) {
		if (timezone === null) {
		...
		if (
			timezoneObject !== Timezone.utc &&
			timezoneObject !== Timezone.floating
		) {
			const calendar = CalendarComponent.fromEmpty();
			calendar.addComponent(
				TimezoneComponent.fromICALJs(timezoneObject.toICALJs())
			);
		...
		const calendar = mapDavCollectionToCalendar(
			response,
			context.getters.getCurrentUserPrincipal
		);
	...
	deleteCalendarAfterTimeout(context, { calendar, countdown = 7 }) {
		...
		const deleteTimeout = setTimeout(async () => {
			try {
				...
				showError(
					t("calendar", "An error occurred, unable to delete the calendar.")
				);
				console.error(error);
			} finally {
	...
	async restoreCalendarObject({ commit, state, getters }, { vobject }) {
		...
		const component = calendarObject.calendarComponent.getFirstComponent(
			vobject.objectType
		);
	...
	async getEventsFromCalendarInTimeRange(context, { calendar, from, to }) {
		context.commit("markCalendarAsLoading", { calendar });
		const response = await calendar.dav.findByTypeInTimeRange(
			"VEVENT",
			from,
			to
		);
		...
		if (context.rootState.settings.showTasks) {
			responseTodo = await calendar.dav.findByTypeInTimeRange(
				"VTODO",
				from,
				to
			);
		...
		for (const r of response.concat(responseTodo)) {
			try {
				...
				console.error(
					`could not convert calendar object of calendar ${calendar.id}`,
					e,
					{
						response: r,
					}
	...
	async importEventsIntoCalendar(context) {
		...
		for (const file of files) {
			...
			const calendarId =
				context.rootState.importFiles.importCalendarRelation[file.id];
			if (calendarId === "new") {
				const displayName =
					file.parser.getName() ||
					t("calendar", "Imported {filename}", {
						filename: file.name,
					});
